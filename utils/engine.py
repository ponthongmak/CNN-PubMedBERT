...
def train(model, model_bert, dataloader, optimizer, criterion, device, origin_ids, used_ids, used_masks, max_chunk, max_len, y_list):
    device = device
    model_bert.eval()
    y_pred, y_true = [], []
    train_loss = 0
    ...
    for step, batch in enumerate(dataloader):
        embedding = [] # our input for the actual model
        ...
        for i in range(len(batch[0])):
            # put data (each sample, which contained sub-samples) into gpu
            b_input_ids  = used_ids[origin_ids.get(int(batch[0][i]))].to(device)
            b_input_mask = used_masks[origin_ids.get(int(batch[0][i]))].to(device)
            # no update gradient for extracting wieghts
            with torch.no_grad(): 
                # BERT output (n,maxlen,768)
                emb = model_bert(input_ids = b_input_ids, attention_mask = b_input_mask)
                # concate multiple sub-sample into one sample (concat_maxlen,768)
                emb = emb.view(-1,768) 
                # zero padding for shorter text to max_chunk*max_len
                emb = F.pad(input=emb, pad=(0,0,0,(max_chunk*max_len)-len(emb)), 
...
def evaluate(model, model_bert, dataloader, optimizer, criterion, device, origin_ids, used_ids, used_masks, max_chunk, max_len, y_list):
    device = device
    model_bert.eval()
    model.eval()
    y_pred, y_true = [], []
        ...
        for step, batch in enumerate(dataloader):
            ...
            for i in range(len(batch[0])):
                ...
                emb = F.pad(input=emb, pad=(0,0,0,(max_chunk*max_len)-len(emb)), 
                            mode='constant', value=0)
                embedding.append(emb)
            # stack list of sample tensor to batch tensor (batch_size, max_chunk*max_len, 768)
            embedding = torch.stack(embedding)
            ## eval main model
...
def test(model, model_bert, dataloader, device, origin_ids, used_ids, used_masks, max_chunk, max_len, y_list, thresholds):
    device = device
    model_bert.eval()
    model.eval()
    y_pred, y_true = [], []
...
def get_threshold(model, model_bert, dataloader, device, origin_ids, used_ids, used_masks, max_chunk, max_len, y_list):
    device = device
    model_bert.eval()
    model.eval()
    y_pred, y_true = [], []
